...
$id_user = $_SESSION['id'] ?? '';
$limit = 8;
$query = "SELECT COUNT(*) as total FROM flowers WHERE category = 1";
...
<div class='container body_content'>
    <h1 class="title-comm p-3"><span class="title-holder">Thiết bị điện tử </span></h1>
    <div class='row product-list'>
        <?php
        try {
            $query = "SELECT * FROM flowers WHERE category = 1 LIMIT :offset, :limit";
            $offset = ($page - 1) * $limit;
    ...
    <div class="pagination d-flex justify-content-center m-4">
        <nav aria-label="Page navigation example  ">
            <ul class="pagination ">
                ...
                if ($page > 1) {
                    echo '<li class="page-item"><a class="page-link" aria-label="Previous"  href="hoa.php?page=' . ($page - 1) . '">
                ...
                for ($i = 1; $i <= $total_pages; $i++) {
                    echo '<li class="page-item ' . ($i == $page ? 'active' : '') . '"><a class="page-link" href="hoa.php?page=' . $i . '">' . $i . '</a></li>';
                }

                // Hiển thị nút Next
                if ($page < $total_pages) {
                    echo '<li class="page-item"><a class="page-link"  aria-label="Next" href="hoa.php?page=' . ($page + 1) . '">
