...
$id_user = $_SESSION['id'] ?? '';
$limit = 8;
$query = "SELECT COUNT(*) as total FROM flowers$flowers WHERE category = 2";
...
<div class='container body_content'>
    ...
    <div class='row product-list'>
        <?php
        try {
            $query = "SELECT * FROM flowers WHERE category = 2 LIMIT :offset, :limit";
            $offset = ($page - 1) * $limit;
    ...
    <div class="pagination d-flex justify-content-center m-4">
        <nav aria-label="Page navigation example  ">
            <ul class="pagination ">
                ...
                if ($page > 1) {
                    echo '<li class="page-item"><a class="page-link" aria-label="Previous"  href="hoachucmung.php?page=' . ($page - 1) . '"><span aria-hidden="true">&laquo;</span>
                                <span class="sr-only">Previous</span></a></li>';
                }

                // Hiển thị nút phân trang
                for ($i = 1; $i <= $total_pages; $i++) {
                    echo '<li class="page-item ' . ($i == $page ? 'active' : '') . '"><a class="page-link" href="hoachucmung.php?page=' . $i . '">' . $i . '</a></li>';
                }

                // Hiển thị nút Next
                if ($page < $total_pages) {
                    echo '<li class="page-item"><a class="page-link"  aria-label="Next" href="hoachucmung.php?page=' . ($page + 1) . '"><span aria-hidden="true">&raquo;</span>
